EESchema Schematic File Version 2
LIBS:splitter-rescue
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:pinConnectors
LIBS:splitter-cache
EELAYER 25 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title "68 Pin Splitter"
Date "2016-07-21"
Rev "mk1.0"
Comp "BADGER Lab"
Comment1 "Author: Alex Dawson-Elli"
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L NI68PinConnector J1
U 1 1 579288CE
P 1250 4050
F 0 "J1" H 1050 550 60  0000 C CNN
F 1 "NI68PinConnector" H 1100 7450 60  0000 C CNN
F 2 "PinConn:NI68PinConnector" H 1250 4050 60  0001 C CNN
F 3 "" H 1250 4050 60  0000 C CNN
	1    1250 4050
	1    0    0    -1  
$EndComp
$Comp
L NI68PinConnector J2
U 1 1 5792891F
P 2450 4050
F 0 "J2" H 2250 550 60  0000 C CNN
F 1 "NI68PinConnector" H 2300 7450 60  0000 C CNN
F 2 "PinConn:NI68PinConnectorMirror" H 2450 4050 60  0001 C CNN
F 3 "" H 2450 4050 60  0000 C CNN
	1    2450 4050
	-1   0    0    -1  
$EndComp
Text Label 1600 4850 0    60   ~ 12
EncoderLA
Text Label 1600 5250 0    60   ~ 12
EncoderLB
Text Label 1650 6750 0    60   ~ 12
UpedalL
Text Label 1600 4350 0    60   ~ 12
EncoderRA
Text Label 1600 5150 0    60   ~ 12
EncoderRB
Text Label 1600 950  0    60   ~ 12
EncoderRZ
Text Label 1600 4750 0    60   ~ 12
EncoderLZ
Text Label 1650 6950 0    60   ~ 12
UpedalR
Text Label 1600 1750 0    60   ~ 12
EncoderCA
Text Label 1600 4950 0    60   ~ 12
EncoderCB
Text Label 1600 1650 0    60   ~ 12
EncoderCZ
Text Notes 3100 1300 0    60   ~ 0
Notes: \nEncoder naming =  Encoder[location][channel]\nLocation[Left:L , Right:R , C:Crank]\nChannel[A:quad1 , B:Quad2 , Z:zero Pulse]\n\nForce torque Naming = FT[location][channel]\nlocation[L, R]\nChannel[U1 - U6]\n
Text Label 1650 6450 0    60   ~ 12
Ucrank
Text Label 1650 7450 0    60   ~ 12
FTLU1
Text Label 1700 3950 0    60   ~ 12
FTLU2
Text Label 1650 7150 0    60   ~ 12
FTLU3
Text Label 1700 3650 0    60   ~ 12
FTLU4
Text Label 1700 3450 0    60   ~ 12
FTLU5
Text Label 1650 6650 0    60   ~ 12
TFLU6
Text Label 1700 4050 0    60   ~ 12
FTRU1
Text Label 1700 3750 0    60   ~ 12
FTRU2
Text Label 1700 3250 0    60   ~ 12
FTRU3
Text Label 1700 3150 0    60   ~ 12
FTRU4
Text Label 1700 2950 0    60   ~ 12
FTRU5
Text Label 1650 6350 0    60   ~ 12
FTRU6
NoConn ~ 1450 750 
NoConn ~ 1450 850 
NoConn ~ 1450 1150
NoConn ~ 1450 1250
NoConn ~ 1450 2550
NoConn ~ 1450 2650
NoConn ~ 1450 2750
NoConn ~ 1450 2250
NoConn ~ 1450 2350
NoConn ~ 1450 2850
NoConn ~ 1450 4450
NoConn ~ 1450 4550
NoConn ~ 1450 4650
NoConn ~ 1450 5350
NoConn ~ 1450 5450
NoConn ~ 1450 5550
NoConn ~ 1450 5750
NoConn ~ 1450 5850
NoConn ~ 1450 6850
NoConn ~ 1450 7250
NoConn ~ 2250 5850
NoConn ~ 2250 5750
NoConn ~ 2250 5550
NoConn ~ 2250 5450
NoConn ~ 2250 4650
NoConn ~ 2250 4550
NoConn ~ 2250 4450
NoConn ~ 2250 2850
$Comp
L VCC #PWR01
U 1 1 5792CB55
P 3550 2550
F 0 "#PWR01" H 3550 2400 50  0001 C CNN
F 1 "VCC" H 3550 2700 50  0000 C CNN
F 2 "" H 3550 2550 50  0000 C CNN
F 3 "" H 3550 2550 50  0000 C CNN
	1    3550 2550
	1    0    0    1   
$EndComp
$Comp
L GND #PWR02
U 1 1 5792CB6F
P 3950 2550
F 0 "#PWR02" H 3950 2300 50  0001 C CNN
F 1 "GND" H 3950 2400 50  0000 C CNN
F 2 "" H 3950 2550 50  0000 C CNN
F 3 "" H 3950 2550 50  0000 C CNN
	1    3950 2550
	1    0    0    -1  
$EndComp
$Comp
L PWR_FLAG #FLG03
U 1 1 5792CD6F
P 3950 2350
F 0 "#FLG03" H 3950 2445 50  0001 C CNN
F 1 "PWR_FLAG" H 3950 2530 50  0000 C CNN
F 2 "" H 3950 2350 50  0000 C CNN
F 3 "" H 3950 2350 50  0000 C CNN
	1    3950 2350
	1    0    0    -1  
$EndComp
$Comp
L PWR_FLAG #FLG04
U 1 1 5792CDD1
P 3550 2350
F 0 "#FLG04" H 3550 2445 50  0001 C CNN
F 1 "PWR_FLAG" H 3550 2530 50  0000 C CNN
F 2 "" H 3550 2350 50  0000 C CNN
F 3 "" H 3550 2350 50  0000 C CNN
	1    3550 2350
	1    0    0    -1  
$EndComp
Wire Wire Line
	1450 4850 2250 4850
Wire Wire Line
	1450 5250 2250 5250
Wire Wire Line
	1450 4750 2250 4750
Wire Wire Line
	2250 2050 1450 2050
Wire Wire Line
	1450 6750 2250 6750
Wire Wire Line
	1450 4350 2250 4350
Wire Wire Line
	1450 5150 2250 5150
Wire Wire Line
	1450 950  2250 950 
Wire Wire Line
	1450 6950 2250 6950
Wire Wire Line
	1450 1750 2250 1750
Wire Wire Line
	1450 1650 2250 1650
Wire Wire Line
	1450 4950 2250 4950
Wire Wire Line
	1450 6450 2250 6450
Wire Wire Line
	1450 7450 2250 7450
Wire Wire Line
	1450 3950 2250 3950
Wire Wire Line
	1450 7150 2250 7150
Wire Wire Line
	1450 3650 2250 3650
Wire Wire Line
	1450 3450 2250 3450
Wire Wire Line
	1450 6650 2250 6650
Wire Wire Line
	1450 4050 2250 4050
Wire Wire Line
	1450 3750 2250 3750
Wire Wire Line
	1450 3250 2250 3250
Wire Wire Line
	1450 3150 2250 3150
Wire Wire Line
	1450 2950 2250 2950
Wire Wire Line
	1450 6350 2250 6350
Wire Wire Line
	1450 1050 2250 1050
Wire Wire Line
	2250 1350 1450 1350
Wire Wire Line
	1450 1550 2250 1550
Wire Wire Line
	1450 1850 2250 1850
Wire Wire Line
	2250 1950 1450 1950
Wire Wire Line
	1450 2150 2250 2150
Wire Wire Line
	2250 2450 1450 2450
Wire Wire Line
	2250 3050 1450 3050
Wire Wire Line
	1450 3350 2250 3350
Wire Wire Line
	2250 3550 1450 3550
Wire Wire Line
	1450 3850 2250 3850
Wire Wire Line
	2250 4150 1450 4150
Wire Wire Line
	1450 4250 2250 4250
Wire Wire Line
	2250 5650 1450 5650
Wire Wire Line
	1450 5950 2250 5950
Wire Wire Line
	2250 6250 1450 6250
Wire Wire Line
	1450 6550 2250 6550
Wire Wire Line
	2250 7350 1450 7350
Wire Wire Line
	1450 5050 2250 5050
Wire Wire Line
	1450 7050 2250 7050
Wire Wire Line
	3550 2350 3550 2550
Wire Wire Line
	3950 2350 3950 2550
Wire Wire Line
	2250 1450 1450 1450
Text Label 1700 1350 0    60   ~ 0
DGnd
Text Label 1700 1550 0    60   ~ 0
DGnd
Text Label 1700 1450 0    60   ~ 0
VCC1
Text Label 1700 1850 0    60   ~ 0
DGnd
Text Label 1700 1950 0    60   ~ 0
DGnd
Text Label 1700 2050 0    60   ~ 0
VCC2
Text Label 1700 2150 0    60   ~ 0
DGnd
Text Label 1700 2450 0    60   ~ 0
DGnd
Text Label 1700 3050 0    60   ~ 0
AI_Gnd
Text Label 1700 1050 0    60   ~ 0
DGnd
Text Label 1700 3350 0    60   ~ 0
AI_Gnd
Text Label 1700 3550 0    60   ~ 0
AI_Gnd
Text Label 1900 750  0    60   ~ 0
PFI_14
Wire Wire Line
	1900 750  2250 750 
Text Label 4600 2400 0    60   ~ 0
PFI_14
Text Label 1700 4150 0    60   ~ 0
DGnd
Text Label 1700 4250 0    60   ~ 0
DGnd
Text Label 1700 5050 0    60   ~ 0
DGnd
Text Label 1650 5950 0    60   ~ 0
DGnd
Text Label 1650 6050 0    60   ~ 0
AO_Gnd
Text Label 1650 6150 0    60   ~ 0
AO_Gnd
Text Label 1650 6250 0    60   ~ 0
AI_Gnd
Text Label 1650 6550 0    60   ~ 0
AI_GND
Text Label 1650 7050 0    60   ~ 0
AI_Gnd
Text Label 1650 7350 0    60   ~ 0
AI_Gnd
Text Label 5800 2300 0    60   ~ 0
DGnd
Text Label 4600 2300 0    60   ~ 0
PFI_12
Wire Wire Line
	4550 2300 5050 2300
Wire Wire Line
	2250 850  1900 850 
Text Label 1900 850  0    60   ~ 0
PFI_12
Wire Wire Line
	4550 2400 5050 2400
Text Label 5800 2400 0    60   ~ 0
EncoderRZ
Wire Wire Line
	4550 2500 5050 2500
Text Label 4600 2500 0    60   ~ 0
EncoderRA
Text Label 5800 2500 0    60   ~ 0
PFI_6
Text Label 5800 2600 0    60   ~ 0
PFI_5
Wire Wire Line
	2250 1150 1900 1150
Wire Wire Line
	2250 1250 1900 1250
Text Label 1900 1150 0    60   ~ 0
PFI_6
Text Label 1900 1250 0    60   ~ 0
PFI_5
Text Label 4600 2600 0    60   ~ 0
EncoderLZ
Wire Wire Line
	5050 2600 4550 2600
Text Label 5800 2700 0    60   ~ 0
EncoderLA
Wire Wire Line
	5050 2700 4550 2700
Text Label 4600 2700 0    60   ~ 0
VCC1
Wire Wire Line
	4550 2800 5050 2800
Text Label 4600 2800 0    60   ~ 0
EncoderCB
Text Label 5800 2800 0    60   ~ 0
EncoderCZ
Text Label 5800 2900 0    60   ~ 0
EncoderRB
Wire Wire Line
	5050 2900 4550 2900
Text Label 4600 2900 0    60   ~ 0
EncoderCA
Wire Wire Line
	5050 3000 4550 3000
Text Label 4600 3000 0    60   ~ 0
EncoderLB
Text Label 1700 5650 0    60   ~ 0
DGnd
Wire Wire Line
	5050 3100 4550 3100
Wire Wire Line
	5050 3200 4550 3200
Text Label 4600 3100 0    60   ~ 0
P0.6
Wire Wire Line
	2250 2250 1950 2250
Wire Wire Line
	2250 2350 1950 2350
Wire Wire Line
	2250 2550 1950 2550
Wire Wire Line
	2250 2650 1950 2650
Text Label 2000 2250 0    60   ~ 0
P0.6
Text Label 2000 2350 0    60   ~ 0
P.1
Text Label 2000 2550 0    60   ~ 0
P0.4
Text Label 2000 2650 0    60   ~ 0
APFI_0
Text Label 5800 3100 0    60   ~ 0
P.1
Text Label 5800 3000 0    60   ~ 0
P0.3
Wire Wire Line
	2250 5350 1850 5350
Text Label 1900 5350 0    60   ~ 0
P0.3
Text Label 4600 3200 0    60   ~ 0
P0.4
Text Label 5800 3200 0    60   ~ 0
APFI_0
Wire Wire Line
	5050 3300 4550 3300
Text Label 4600 3300 0    60   ~ 0
AO_1
Wire Wire Line
	2250 2750 1950 2750
Text Label 2000 2750 0    60   ~ 0
AO_1
Text Label 5800 3300 0    60   ~ 0
AO_Gnd
Wire Wire Line
	5050 3400 4550 3400
$Comp
L PinHeaders J3
U 1 1 5792E064
P 5400 5500
F 0 "J3" H 5450 6700 60  0000 C CNN
F 1 "PinHeaders" H 5400 8850 60  0000 C CNN
F 2 "PinConn:Pin_Header_Straight_2x20" H 5400 5500 60  0001 C CNN
F 3 "" H 5400 5500 60  0000 C CNN
	1    5400 5500
	1    0    0    -1  
$EndComp
Wire Wire Line
	5050 3500 4550 3500
Wire Wire Line
	5050 3600 4550 3600
Wire Wire Line
	5050 3700 4550 3700
Wire Wire Line
	5050 3800 4550 3800
Wire Wire Line
	4550 3900 5050 3900
Wire Wire Line
	4550 4000 5050 4000
Wire Wire Line
	5050 4100 4550 4100
Wire Wire Line
	5050 4200 4550 4200
Text Label 4600 3400 0    60   ~ 0
FTRU6
Text Label 5800 3400 0    60   ~ 0
FTRU5
Text Label 5800 3500 0    60   ~ 0
UCrank
Text Label 4600 3500 0    60   ~ 0
TFLU6
Text Label 4600 3600 0    60   ~ 0
FTRU4
Text Label 5800 3600 0    60   ~ 0
FTRU3
Text Label 5800 3700 0    60   ~ 0
UpedalL
Text Label 4600 3700 0    60   ~ 0
FTLU5
Text Label 4600 3800 0    60   ~ 0
UpedalR
Text Label 5800 3800 0    60   ~ 0
FTLU4
Text Label 4600 3900 0    60   ~ 0
FTLU3
Text Label 5800 3900 0    60   ~ 0
FTRU2
Text Label 1700 3850 0    60   ~ 0
AI_GND
Wire Wire Line
	1850 7250 2250 7250
Text Label 1850 7250 0    60   ~ 0
AI_9
Text Label 5800 4000 0    60   ~ 0
AI_9
Text Label 5800 4200 0    60   ~ 0
AI_GND
Text Label 4600 4100 0    60   ~ 0
FTLU1
Text Label 5800 4100 0    60   ~ 0
FTRU1
Text Label 1850 6850 0    60   ~ 0
AI_SENSE
Wire Wire Line
	1850 6850 2250 6850
Text Label 4600 4200 0    60   ~ 0
AI_SENSE
Text Label 4600 4000 0    60   ~ 0
FTLU2
Wire Wire Line
	2250 6050 1650 6050
Wire Wire Line
	2250 6150 1650 6150
$EndSCHEMATC
